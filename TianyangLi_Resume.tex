%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Deedy CV/Resume
% XeLaTeX Template
% Version 1.0 (5/5/2014)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Debarghya Das (http://www.debarghyadas.com)
% With extensive modifications by:
% Vel (vel@latextemplates.com)
%
% License:
% CC BY-NC-SA 3.0 (http://creativecommons.org/licenses/by-nc-sa/3.0/)
%
% Important notes:
% This template needs to be compiled with XeLaTeX.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[letterpaper]{deedy-resume} % Use US Letter paper, change to a4paper for A4 

\begin{document}

%----------------------------------------------------------------------------------------
%	TITLE SECTION
%----------------------------------------------------------------------------------------

\lastupdated % Print the Last Updated text at the top right

\namesection{Tianyang}{Li}{Graduate Student at University of Oxford, Mathematical Sciences \\
\href{mailto:tianyang.li@linacre.ox.ac.uk}{tianyang.li@linacre.ox.ac.uk}}

%----------------------------------------------------------------------------------------
%	LEFT COLUMN
%----------------------------------------------------------------------------------------

\begin{minipage}[t]{0.33\textwidth} % The left column takes up 33% of the text width of the page

%------------------------------------------------
% Links
%------------------------------------------------

\section{Links} 
Website: \href{https://www.l-ty.com}{\bf www.l-ty.com} \\
Github: \href{https://github.com/litianyang0211}{\bf github.com/litianyang0211} \\
%LinkedIn: \href{https://www.linkedin.com/in/tianyu-du}{\bf linkedin.com/in/tianyu-du} \\
%YouTube:// \href{https://www.youtube.com/user/DeedyDash007}{\bf DeedyDash007} \\
%Twitter:// \href{https://twitter.com/debarghya_das}{\bf @debarghya\_das} \\
%Quora:// \href{https://www.quora.com/Debarghya-Das}{\bf Debarghya-Das}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Coursework
%------------------------------------------------

\section{Coursework}
\subsection{Graduate}
\location{(University of Oxford, in progress)}
Network \\
Theory of Deep Learning \\
Statistical Machine Learning \\
Simulation Method \\
Graphical Model \\

\sectionspace

\location{(University of Toronto)}
Stochastic Process (A+)\\%, 99\%) \\
Method of Applied Statistics (A+) \\
Probabilistic Machine Learning (A+)\\%, 100\%) \\
Statistical Computation (A+) \\

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Undergraduate}
\location{(University of Toronto)}
Theory of Statistical Practice (A+)\\%, 91\%)
Machine Learning (A+) \\
Method of Data Analysis (A+) \\
Probability (A+) \\
Econometrics (A+) \\
Intermediate Microeconomics (A+) \\
Intermediate Macroeconomics (A+) \\
Real Analysis (A+)\\%, 91\%) \\
Chaos, Fractals and Dynamics (A+) \\
Abstract Mathematics (A) \\
Ordinary Differential Equation (A+) \\
Linear Algebra (A+)\\
Calculus (A+)\\

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Skills
%------------------------------------------------

\section{Skills}
\subsection{Programming}
Julia \textbullet{} Matlab \textbullet{} Octave \\
Python \textbullet{} R \textbullet{} Stata \\

\sectionspace

\subsection{Data Science Libraries}
Numpy \textbullet{} Pandas \textbullet{} PyTorch \\
Sci-kit Learn \\

\sectionspace

\subsection{Others}
\LaTeX

\sectionspace % Some whitespace after the section

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the left column
\hfill
%
%----------------------------------------------------------------------------------------
%	RIGHT COLUMN
%----------------------------------------------------------------------------------------
%
\begin{minipage}[t]{0.66\textwidth} % The right column takes up 66% of the text width of the page

%------------------------------------------------
% Education
%------------------------------------------------

\section{Education}

\runsubsection{University of Oxford}
\descript{| Oct 2021 - Jun 2022 (Expected)}
\location{Master of Science, Mathematical Sciences}
%\location{Expected Sep 2020 – Jun 2022}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightitemize}
\item Supervisor: Dr. Andrey Kormilitzin \& Prof. Matthias Winkel
\item Focus on natural language processing
\item A member of chronosig research group, which is funded by the National Institute of Health Research.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{University of Toronto}
\descript{| Sep 2017 - Jun 2021}
\location{Honours Bachelor of Science (High Distinction), Statistics (Specialist) \& \\ Mathematics (Minor)}
\begin{tightitemize}
\item Cum. GPA: 3.97/4.00, course average: 94\%
%\item Top graduating student from college.
%\item Top graduating student from the department of.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Peking University}
\descript{| Jun 2019 - Jul 2019}
\location{Summer Exchange, Modern Machine Learning in Practice}

%------------------------------------------------
% Awards
%------------------------------------------------

\section{Honours} 

\begin{tabular}{rll}
Jun 2021	 & Walter Neil Thompson McKay Scholarship \\
Dec 2020	 & Faculty of Arts \& Science Alumni \& Friends Undergraduate Scholarship \\
Aug 2020	 & Joseph Wesley MacCallum Scholarship \\
Dec 2018	 & Samuel Beatty In-Course Scholarship \\
Nov 2018	 & Lawrence and Sharen Ho International Scholarship\\
Oct 2018	 & James Morrow Scholarship \\
2017-20		 & Dean's List Scholar (All Semesters at University of Toronto)
\end{tabular}

\sectionspace % Some whitespace after the section

%\sectionspace % Some whitespace after the section

%------------------------------------------------
% Research
%------------------------------------------------

\section{Research Experience}
\runsubsection{Modified Batch Renormalization Algorithm}
\descript{| Dec 2021}
\small{We proposed a novel Batch Renormalization method, named Modified Batch Renormalization (MBR), that overcame insufficient mini-batch problem without introducing additional nonlinear operations. The core idea of MBR is to substitute exponential moving average (EMA) statistics for batch statistics and modify the EMA statistics.}

\sectionspace

\runsubsection{Real-World Application of True Posterior Approximation}
\descript{| Mar 2020}
\small{We implemented a variant of the TrueSkill model using gradient-based stochastic variational inference, optimized the approximate posterior to estimate the true posterior with tennis match outcomes and analyzed the framework of athletes' skill sets.}

\sectionspace

\runsubsection{TD Rotman FinHub TDMDAL Hackathon}
\descript{| Feb 2020}
\location{Finalist Group (Top 5)}
\small {We developed a dictionary-based NLP platform to extract information from transcripts of earning calls of S\&P 500 companies, and predict stock price fluctuation on the next trading day.}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Teaching
%------------------------------------------------

\section{Teaching Experience}
\runsubsection{Methods of Data Analysis}
\descript{| Tutor | Jan 2021 - Apr 2021}

\sectionspace

\runsubsection{The Practice of Statistics}
\descript{| Tutor | May 2020 - Dec 2020}

\sectionspace

\runsubsection{Calculus II}
\descript{| Teaching Assistant | Jan 2019 - Apr 2019}

\sectionspace

\runsubsection{Calculus I}
\descript{| Teaching Assistant | Sep 2018 - Dec 2018}

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------
%	SECOND PAGE (EXAMPLE)
%----------------------------------------------------------------------------------------

%\newpage % Start a new page

%\begin{minipage}[t]{0.33\textwidth} % The left column takes up 33% of the text width of the page

%\section{Example Section}

%\end{minipage} % The end of the left column
%\hfill
%\begin{minipage}[t]{0.66\textwidth} % The right column takes up 66% of the text width of the page

%\section{Example Section 2}

%\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------

\end{document}